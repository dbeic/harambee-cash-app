...
# Add this with your other global variables (around line where stop_event is defined)
...
# Database initialization function - Add this to your main app file
def init_db():
    with get_db_connection() as conn:
        ...
        # System accounting table - CRITICAL for money conservation
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS system_accounting (
                id INTEGER PRIMARY KEY,
                total_system_money NUMERIC DEFAULT 0,
        ...
        # Initialize system accounting if not exists
        cursor.execute("SELECT COUNT(*) FROM system_accounting WHERE id = 1")
        if cursor.fetchone()[0] == 0:
            cursor.execute("SELECT COALESCE(SUM(wallet), 0) FROM users")
            total_money = cursor.fetchone()[0] or 0
            cursor.execute("""
                INSERT INTO system_accounting (id, total_system_money, total_deposited, total_withdrawn, total_fees_collected, games_played)
                VALUES (1, %s, 0, 0, 0, 0)
            """, (total_money,))
        
        # Insert default withdrawal fees if table is empty
        ...
        indexes = [
            "CREATE INDEX IF NOT EXISTS idx_game_queue_user_id ON game_queue(user_id)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_transactions_game_code ON transactions(game_code)",
            "CREATE INDEX IF NOT EXISTS idx_results_timestamp ON results(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_results_game_code ON results(game_code)",
            "CREATE INDEX IF NOT EXISTS idx_withdrawal_requests_status ON withdrawal_requests(status)",
            "CREATE INDEX IF NOT EXISTS idx_withdrawal_requests_timestamp ON withdrawal_requests(request_time)",
            "CREATE INDEX IF NOT EXISTS idx_deposit_requests_status ON deposit_requests(status)",
            "CREATE INDEX IF NOT EXISTS idx_deposit_requests_timestamp ON deposit_requests(request_time)",
            "CREATE INDEX IF NOT EXISTS idx_deposit_requests_voucher_code ON deposit_requests(voucher_code)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_win_earnings_user_id ON win_earnings(user_id)",
            "CREATE INDEX IF NOT EXISTS idx_win_earnings_game_code ON win_earnings(game_code)",
            "CREATE INDEX IF NOT EXISTS idx_win_earnings_withdrawn ON win_earnings(is_withdrawn)",
            "CREATE INDEX IF NOT EXISTS idx_system_accounting_id ON system_accounting(id)"
        ]
...
#@login_required()
@limiter.limit("3 per minute")  # More generous limit
def play():
...
def admin_dashboard():
    with get_db_connection() as conn:
        ...
        cursor.execute(
            ...
            "WHERE suspension_end > CURRENT_TIMESTAMP"
        )
...
@admin_required
def audit_system():
    """Complete system audit - verifies money is conserved"""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            
            # Get all money in users
            cursor.execute("SELECT COALESCE(SUM(wallet), 0) FROM users")
            total_in_users = float(cursor.fetchone()[0])
            ...
            """)
            result = cursor.fetchone()
            deposits = float(result[0]) if result else 0.0
            withdrawals = float(result[1]) if result else 0.0
            
            # Get system accounting - handle case where table doesn't exist
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS system_accounting (
                    id INTEGER PRIMARY KEY,
                    total_system_money NUMERIC DEFAULT 0,
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
                    INSERT INTO system_accounting (id, total_system_money, total_deposited, total_withdrawn)
                    VALUES (1, %s, %s, %s)
                """, (total_in_users, deposits, withdrawals))
                games_played = 0
                total_fees = 0
            else:
                cursor.execute("SELECT games_played, total_fees_collected FROM system_accounting WHERE id = 1")
                acc_data = cursor.fetchone()
                games_played = acc_data[0] if acc_data else 0
                total_fees = float(acc_data[1]) if acc_data and acc_data[1] else 0.0
            
            # The GOLDEN RULE: total_in_users should equal deposits - withdrawals
            expected_total = deposits - withdrawals
            difference = total_in_users - expected_total
            
            # Check each game for conservation
            cursor.execute("""
                SELECT game_code, total_amount, deduction, winner_amount, 
                       (total_amount - deduction - winner_amount) as error
                FROM results 
                WHERE status = 'completed'
                HAVING ABS(total_amount - deduction - winner_amount) > 0.01
            """)
            bad_games = cursor.fetchall()
            
            # Get recent games
            cursor.execute("""
                SELECT game_code, timestamp, num_users, total_amount, winner, winner_amount, deduction
                FROM results 
                WHERE status = 'completed'
                ORDER BY timestamp DESC
                LIMIT 10
            ...
            for game in recent:
                recent_games.append({
                    ...
                    "timestamp": game[1],
                    "num_users": game[2],
                    "total_amount": float(game[3]) if game[3] else 0,
                    "winner": game[4] if game[4] else "N/A",
                    "winner_amount": float(game[5]) if game[5] else 0,
                    "deduction": float(game[6]) if game[6] else 0
                })
            
            return jsonify({
                'timestamp': datetime.now().isoformat(),
                'total_system_money': total_in_users,
                'total_deposits': deposits,
                'total_withdrawals': withdrawals,
                'expected_total': expected_total,
                'difference': difference,
                'accounting_perfect': abs(difference) < 0.01,
                'games_played': games_played,
                'total_fees': total_fees,
                'problem_games': len(bad_games),
                'problem_games_list': bad_games,
                'recent_games': recent_games,
                'message': '✅ Money is conserved!' if abs(difference) < 0.01 else '❌ Money leak detected!'
            })
            
    except Exception as e:
        logging.error(f"Audit error: {e}")
        return jsonify({'error': str(e), 'message': 'Audit failed'}), 500
...
<head>
    ...
    <style>
        ...
        .total-amount { color: var(--info-blue); }
        .deposit-amount { color: var(--success-green); }
        .withdraw-amount { color: var(--danger-red); }
        .fee-amount { color: var(--warning-orange); }
...
<body>
    <div class="container">
        <!-- Header -->
        <div class="header">
            <h1><i class="fas fa-chart-line"></i> HARAMBEE CASH - Financial Dashboard</h1>
            <div class="header-subtitle">Money Conservation Verification System</div>
        ...
        <div class="golden-rule">
            <span>💰 GOLDEN RULE:</span> Total Money = Deposits - Withdrawals 
            {% if data and data.get('accounting_perfect') %}
                <span class="verification-badge">✓ VERIFIED</span>
            {% else %}
                <span class="verification-badge" style="background: var(--danger-red);">⚠ CHECK</span>
            {% endif %}
        </div>
        ...
        <div class="financial-overview">
            <div class="overview-card">
                <h3>Total System Money</h3>
                <div class="amount total-amount">Ksh. {{ "%.2f"|format(data.total_system_money|default(0)) }}</div>
                <div class="description">Actual money in all user wallets</div>
            </div>
            
            <div class="overview-card">
                <h3>Total Deposits</h3>
                <div class="amount deposit-amount">Ksh. {{ "%.2f"|format(data.total_deposits|default(0)) }}</div>
                <div class="description">Money added to system from outside</div>
            </div>
            
            <div class="overview-card">
                <h3>Total Withdrawals</h3>
                <div class="amount withdraw-amount">Ksh. {{ "%.2f"|format(data.total_withdrawals|default(0)) }}</div>
                <div class="description">Money removed from system</div>
            </div>
            
            <div class="overview-card">
                <h3>Fees Collected</h3>
                <div class="amount fee-amount">Ksh. {{ "%.2f"|format(data.total_fees|default(0)) }}</div>
                <div class="description">Platform fees from {{ data.games_played|default(0) }} games</div>
            </div>
        </div>
        
        <!-- Accounting Verification -->
        <div class="verification-section">
            <h2>🔍 Accounting Verification</h2>
            <table class="verification-table">
                <tr>
                    <td>Total User Money (A)</td>
                    <td>Ksh. {{ "%.2f"|format(data.total_system_money|default(0)) }}</td>
                </tr>
                <tr>
                    <td>Total Deposits (B)</td>
                    <td>Ksh. {{ "%.2f"|format(data.total_deposits|default(0)) }}</td>
                </tr>
                <tr>
                    <td>Total Withdrawals (C)</td>
                    <td>Ksh. {{ "%.2f"|format(data.total_withdrawals|default(0)) }}</td>
                </tr>
                <tr>
                    <td>Expected Total (B - C)</td>
                    <td>Ksh. {{ "%.2f"|format((data.total_deposits|default(0)) - (data.total_withdrawals|default(0))) }}</td>
                </tr>
                <tr>
                    <td>Difference (A - (B - C))</td>
                    <td class="{% if data.difference|default(0) < 0.01 %}match-yes{% else %}match-no{% endif %}">
                        Ksh. {{ "%.4f"|format(data.difference|default(0)) }}
                        {% if data.difference|default(0) < 0.01 %}
                            ✓ PERFECT
                        {% else %}
                            ⚠ MISMATCH
                        {% endif %}
            ...
            <div class="conservation-message">
                {% if data.difference|default(0) < 0.01 %}
                    ✅ Money is perfectly conserved! Games only redistribute, never create or destroy.
                {% else %}
                    ❌ CRITICAL: Money leak detected! {{ "%.4f"|format(data.difference|default(0)) }} created/destroyed!
                {% endif %}
            </div>
        </div>
        ...
        <div class="key-metrics">
            ...
            <div class="metrics-grid">
                <div class="metric-item">
                    ...
                    <span class="metric-value">{{ data.games_played|default(0) }}</span>
                </div>
                <div class="metric-item">
                    <span class="metric-label">Total Fees Collected</span>
                    <span class="metric-value">Ksh. {{ "%.2f"|format(data.total_fees|default(0)) }}</span>
                </div>
                <div class="metric-item">
                    <span class="metric-label">Average Fee per Game</span>
                    <span class="metric-value">
                        {% if data.games_played|default(0) > 0 %}
                            Ksh. {{ "%.2f"|format((data.total_fees|default(0)) / data.games_played) }}
                        {% else %}
                            Ksh. 0.00
                        {% endif %}
                    </span>
                ...
                <div class="metric-item">
                    <span class="metric-label">Net Flow (Deposits - Withdrawals)</span>
                    <span class="metric-value">Ksh. {{ "%.2f"|format((data.total_deposits|default(0)) - (data.total_withdrawals|default(0))) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="recent-games">
            <h2>🎮 Recent Games (Money Conserved)</h2>
            {% if data.recent_games and data.recent_games|length > 0 %}
            <table class="games-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ game.timestamp.strftime('%H:%M %d/%m') if game.timestamp else 'N/A' }}</td>
                        <td>{{ game.num_users|default(0) }}</td>
                        <td>Ksh. {{ "%.2f"|format(game.total_amount|default(0)) }}</td>
                        <td>{{ game.winner|default('N/A') }}</td>
                        <td>Ksh. {{ "%.2f"|format(game.winner_amount|default(0)) }}</td>
                        <td><span class="fee-badge">Ksh. {{ "%.2f"|format(game.deduction|default(0)) }}</span></td>
                        <td>✅ Completed</td>
                    </tr>
                    {% endfor %}
                </tbody>
            ...
            {% else %}
            <div class="no-data">
                <i class="fas fa-inbox"></i> No games recorded yet. Game statistics will appear here as games are completed.
            </div>
            {% endif %}
        </div>
        ...
        <div class="financial-health">
            ...
            <div class="health-message">
                <p><strong>Money Conservation Law:</strong> The total money in the system (Ksh. {{ "%.2f"|format(data.total_system_money|default(0)) }}) 
                is exactly equal to total deposits (Ksh. {{ "%.2f"|format(data.total_deposits|default(0)) }}) minus total withdrawals (Ksh. {{ "%.2f"|format(data.total_withdrawals|default(0)) }}).</p>
                
                <p><strong>Games are redistribution mechanisms only.</strong> When players play, money simply moves from losers to winners.
                The platform collects fees (Ksh. {{ "%.2f"|format(data.total_fees|default(0)) }} total), but this is accounted for in the game math.</p>
                
                <p><strong>Verification:</strong> Difference of Ksh. {{ "%.4f"|format(data.difference|default(0)) }} between actual and expected.
                {% if data.difference|default(0) < 0.01 %}
        ...
        <div class="footer-actions">
            ...
            <a href="/api/admin/audit" class="action-btn audit-btn" target="_blank">
    ...
    <script>
        ...
        function updateTimestamp() {
            ...
            if (timeElement) {
                timeElement.textContent = now.toLocaleString('en-KE', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
...
<body>
    ...
    <header class="admin-header fade-in">
        ...
        <div class="admin-info">
            <p><i class="fas fa-user-shield"></i> Logged in as: <strong>{{ session.get('admin_username', 'Admin') }}</strong></p>
            <p><i class="fas fa-clock"></i> Session: Active</p>
        </div>
    </header>
    ...
    <div class="stats-dashboard fade-in">
        ...
        <div class="stat-card balance">
            <div class="stat-icon"><i class="fas fa-wallet"></i></div>
            <div class="stat-value">Ksh {{ "%.2f"|format(total_balance|default(0)) }}</div>
            <div class="stat-label">Total Platform Balance</div>
        </div>
        
        <div class="stat-card active">
            <div class="stat-icon"><i class="fas fa-gamepad"></i></div>
            <div class="stat-value">{{ active_games|default(0) }}</div>
            <div class="stat-label">Active Games</div>
        </div>
        
        <div class="stat-card games">
            <div class="stat-icon"><i class="fas fa-trophy"></i></div>
            <div class="stat-value">{{ completed_games|default(0) }}</div>
            <div class="stat-label">Games Completed</div>
        </div>
    </div>
    ...
    <section class="quick-actions fade-in">
        <h2><i class="fas fa-bolt"></i> Quick Actions</h2>
        <div class="action-grid">
            <a href="/cashbook" class="action-btn" target="_blank">
                <i class="fas fa-chart-line"></i>
                <div class="action-text">
                    <h3>Financial Dashboard</h3>
                    <p>View platform profits & financial reports</p>
            ...
            <a href="#" onclick="showTab('userManagement'); return false;" class="action-btn">
                <i class="fas fa-user-cog"></i>
                <div class="action-text">
                    <h3>User Management</h3>
                    <p>Add, edit, or suspend users</p>
            ...
            <a href="#" onclick="showTab('walletManagement'); return false;" class="action-btn">
                <i class="fas fa-coins"></i>
                <div class="action-text">
                    <h3>Wallet Controls</h3>
                    <p>Deposit or withdraw user funds</p>
            ...
            <a href="/admin/deposits" class="action-btn">
    ...
    <div class="main-content fade-in">
        ...
        <div id="userManagement" class="tab-content active">
            ...
            <div class="form-section">
                <h3><i class="fas fa-list"></i> All Users ({{ users|length if users else 0 }})</h3>
                <div style="overflow-x: auto; max-height: 500px; overflow-y: auto;">
                    <table class="data-table" id="userTable">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            {% if users %}
                                {% for user in users %}
                                <tr>
                                    <td>{{ user[0] if user[0] else 'N/A' }}</td>
                                    <td><strong>{{ user[2] if user[2] else 'N/A' }}</strong></td>
                                    <td>{{ user[1] if user[1] else 'N/A' }}</td>
                                    <td><span class="wallet-badge">Ksh {{ (user[4]|default(0)) | round(2) }}</span></td>
                                    <td>
                                        {% if user_suspensions and user_suspensions.get(user[0]) %}
                                        <span class="status-badge status-suspended">Suspended</span>
                                        {% else %}
                                        <span class="status-badge status-active">Active</span>
                                        {% endif %}
                                    ...
                                    <td>
                                        <button class="btn btn-warning btn-sm" onclick="manageUser({{ user[0] if user[0] else 0 }})" 
                                                style="padding: 5px 10px; font-size: 0.85rem;">
                                            <i class="fas fa-edit"></i> Manage
                                        </button>
                                    </td>
            ...
            <div class="form-section">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        {% for allowed in allowed_users %}
                        <tr>
                            <td>{{ allowed[0] if allowed[0] else 'N/A' }}</td>
                            <td>{{ allowed[1] if allowed[1] else 'N/A' }}</td>
                            <td>
                                {% if allowed[2] %}
                                <span class="status-badge status-active">Used</span>
                    ...
                    <table class="data-table" id="activityTable">
                        ...
                        <tbody>
                            {% if logs %}
                                {% for log in logs %}
                                <tr>
                                    <td>{{ log[1].strftime('%Y-%m-%d %H:%M:%S') if log[1] else 'N/A' }}</td>
                                    <td><strong>{{ log[2] or 'Guest' }}</strong></td>
                                    <td>{{ log[3] or 'N/A' }}</td>
                                    <td>{{ log[4] or 'N/A' }}</td>
                                    <td>
    ...
    <script>
        ...
        async function fetchUserDetails(userId) {
            ...
            document.getElementById('userName').innerHTML = '<span class="loading"></span> Loading...';
            
            try {
                const response = await fetch(`/admin/user_details/${userId}`);
                if (response.ok) {
                    const data = await response.json();
                    document.getElementById('userName').textContent = `User: ${data.username || 'Unknown'} (ID: ${userId})`;
                    document.getElementById('currentBalance').textContent = `Ksh ${parseFloat(data.balance || 0).toFixed(2)}`;
                    currentUserBalance = parseFloat(data.balance || 0);
                    updateAmountLabel();
                } else {
                    document.getElementById('userName').textContent = `User ID ${userId} not found`;
                    currentUserBalance = 0;
                }
            } catch (error) {
                console.error('Error fetching user details:', error);
                document.getElementById('userName').textContent = 'Error loading user details';
                currentUserBalance = 0;
            }
        }
        ...
        function manageUser(userId) {
            ...
            modal.innerHTML = `
                <div style="background: var(--card-bg); padding: 30px; border-radius: 15px; max-width: 500px; width: 90%;">
                    <h2 style="color: var(--accent-gold); margin-bottom: 20px;">Manage User ${userId}</h2>
                    <div style="margin-bottom: 20px;">
                        <button class="btn btn-warning" style="width: 100%; margin-bottom: 10px;" onclick="alert('Edit user functionality coming soon')">
                            <i class="fas fa-edit"></i> Edit User Details
                        </button>
                        <button class="btn btn-danger" style="width: 100%; margin-bottom: 10px;" onclick="alert('Suspend user functionality coming soon')">
                            <i class="fas fa-ban"></i> Suspend User
                        </button>
                        <button class="btn btn-primary" style="width: 100%; margin-bottom: 10px;" onclick="alert('Transaction history coming soon')">
                            <i class="fas fa-history"></i> View Transaction History
                        </button>
                    </div>
                    <button onclick="this.parentElement.parentElement.remove()" class="btn" 
        ...
        function removeAllowedUser(username) {
            if (confirm(`Are you sure you want to remove "${username}" from allowed users?`)) {
                fetch('/admin/remove_allowed_user', {
                    ...
                    body: `username=${encodeURIComponent(username)}&csrf_token={{ csrf_token() if csrf_token else '' }}`
                }).then(response => {
                    if (response.ok) {
                        location.reload();
                    } else {
...
<head>
    ...
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Documentation | Harambee Cash API</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        .container {
            ...
            border-radius: 30px;
            padding: 50px;
            box-shadow: 0 0 50px rgba(0,255,0,0.2), 0 0 0 2px #238636;
...
<body>
    <div class="container">
        <div class="dev-badge">
            <span>⚡ HARAMBEE CASH API v2.0</span>
            <span class="award">🏆 BEST FINTECH API 2025 🏆</span>
        </div>
        ...
        <h2 style="color: #2EA043; margin: 40px 0 20px;">🔌 API ENDPOINTS</h2>
        
        <div class="endpoint-grid">
            <div class="endpoint">
                <span class="endpoint-method">GET</span>
                <span class="endpoint-path">/</span>
                <div class="endpoint-desc">Homepage & API status</div>
            </div>
            <div class="endpoint">
            ...
            <div class="endpoint">
                ...
                <div class="endpoint-desc">Authenticate user</div>
            </div>
            <div class="endpoint">
                <span class="endpoint-method">POST</span>
                <span class="endpoint-path">/play</span>
                <div class="endpoint-desc">Enroll in game</div>
            </div>
            <div class="endpoint">
                <span class="endpoint-method">GET</span>
                <span class="endpoint-path">/wallet</span>
                <div class="endpoint-desc">Check balance</div>
            </div>
            <div class="endpoint">
                <span class="endpoint-method">POST</span>
                <span class="endpoint-path">/withdraw</span>
                <div class="endpoint-desc">Request withdrawal</div>
            </div>
        </div>
        
        <h2 style="color: #2EA043; margin: 40px 0 20px;">✨ CORE FEATURES</h2>
        
        <div class="feature-list">
            <div class="feature-item">
                <div class="feature-icon">👥</div>
                <h3>User Management</h3>
                <p>Secure registration, 2FA, profile management</p>
            </div>
            <div class="feature-item">
                <div class="feature-icon">💰</div>
                <h3>Wallet System</h3>
                <p>Real-time balance, transactions, M-Pesa integration</p>
            </div>
            <div class="feature-item">
                <div class="feature-icon">🎮</div>
                <h3>Game Engine</h3>
                <p>30-second rounds, random winner selection</p>
            </div>
            <div class="feature-item">
                <div class="feature-icon">📊</div>
                <h3>Admin Dashboard</h3>
                <p>Analytics, user management, reports</p>
            </div>
            <div class="feature-item">
                <div class="feature-icon">🔐</div>
                <h3>Security</h3>
                <p>bcrypt hashing, SQL injection protection</p>
            </div>
            <div class="feature-item">
                <div class="feature-icon">📱</div>
                <h3>Mobile Ready</h3>
                <p>Responsive API, React Native SDK coming</p>
            </div>
        </div>
        
        <h2 style="color: #2EA043; margin: 40px 0 20px;">📊 DATABASE SCHEMA</h2>
        
        <div class="code-block">
            <pre>users {<br>  id: UUID (primary)<br>  email: string (unique)<br>  username: string<br>  password_hash: string<br>  wallet_balance: decimal<br>  created_at: timestamp<br>  last_login: timestamp<br>}<br><br>transactions {<br>  id: UUID<br>  user_id: UUID (foreign)<br>  amount: decimal<br>  type: enum(deposit,withdrawal,winning)<br>  status: enum(pending,completed,failed)<br>  timestamp: timestamp<br>}</pre>
        </div>
        
        <h2 style="color: #2EA043; margin: 40px 0 20px;">🚀 FUTURE ENHANCEMENTS</h2>
        
        <div class="future-enhancements">
            <div class="enhancement-item">📱 Native Mobile Apps</div>
            <div class="enhancement-item">🤖 AI Support Chatbot</div>
            <div class="enhancement-item">🔐 2FA for All</div>
            <div class="enhancement-item">📈 Leaderboards</div>
            <div class="enhancement-item">👥 Referral System</div>
            <div class="enhancement-item">🌐 Multi-language</div>
            <div class="enhancement-item">💳 M-Pesa Auto</div>
            <div class="enhancement-item">📊 Analytics Dashboard</div>
            <div class="enhancement-item">🎯 Achievement Badges</div>
            <div class="enhancement-item">📧 Email Verification</div>
            <div class="enhancement-item">🤝 Affiliate Program</div>
            <div class="enhancement-item">📱 React Native SDK</div>
        </div>
        
        <footer>
            <p>© 2025 Pigasimu. Open Source | MIT License</p>
            <p style="margin-top: 10px;">⚡ Built by developers, for developers ⚡</p>
        </footer>
    ...
    <style>
        * {
        ...
        .container {
            ...
            border: 1px solid rgba(67, 198, 172, 0.5);
        }
        ...
        .data-card li {
            padding: 10px 0;
            border-bottom: 1px dashed #43C6AC;
            color: #2C3E50;
        }
...
<body>
    <div class="container">
        <div class="privacy-shield">🛡️ GDPR & KENYA COMPLIANT 🛡️</div>
        ...
        <div class="privacy-badge">
            <div class="badge-item">🔐 256-BIT ENCRYPTION</div>
            <div class="badge-item">🇰🇪 DATA SOVEREIGNTY</div>
            <div class="badge-item">⚡ REAL-TIME PROTECTION</div>
        </div>
        ...
        <div class="data-grid">
            ...
            <div class="data-card">
                <h3>🛡️ HOW WE PROTECT YOU</h3>
                <ul>
                    <li>✓ Bank-level encryption</li>
                    <li>✓ 24/7 Security Monitoring</li>
                    <li>✓ Regular Security Audits</li>
                    <li>✓ Biometric authentication</li>
                    <li>✓ Instant breach notification</li>
                </ul>
            </div>
        ...
        <div class="trust-seal">
            ...
            <p style="font-size: 18px; margin: 20px 0;">Over 100,000 Kenyan gamers trust us with their data daily</p>
            <p>📧 privacy@harambee-cash.co.ke | 📞 +254 700 000001</p>
        </div>
        
        <footer style="text-align: center; color: #666;">
            <p>© 2025 Pigasimu. Your privacy is our priority.</p>
        </footer>
    ...
    <style>
        * {
        ...
        body {
            ...
            background: linear-gradient(145deg, #0B4F6C 0%, #145C8F 50%, #1B3B6F 100%);
            min-height: 100vh;
            padding: 30px 20px;
            position: relative;
        }
        ...
        .container {
            ...
            padding: 50px;
            box-shadow: 0 40px 70px rgba(0,0,0,0.4), 0 0 0 2px #C5A028 inset, 0 0 0 5px rgba(197, 160, 40, 0.3) inset;
            position: relative;
        ...
        @keyframes badgePulse {
            ...
            50% { transform: rotate(3deg) scale(1.05); box-shadow: 0 15px 40px rgba(255,215,0,0.9); }
        }
        ...
        h3 {
            ...
            margin: 30px 0 15px;
            color: #0B4F6C;
            background: linear-gradient(to right, rgba(11,79,108,0.1), transparent);
            padding: 12px 25px;
        ...
        .legal-footer {
            margin-top: 50px;
            padding: 30px;
            background: linear-gradient(145deg, #1B3B6F, #0B4F6C);
            border-radius: 30px;
            color: white;
            text-align: center;
        }
...
<body>
    <div class="container">
        <div class="award-badge">🏆 2025 WINNER 🏆</div>
        ...
        <div style="text-align: center; margin: 20px 0;">
            <span style="background: #1B3B6F; color: white; padding: 8px 25px; border-radius: 50px;">📅 Effective: February 6, 2025</span>
        </div>
        
        <p style="font-size: 18px; text-align: center; font-weight: 500;">Welcome to the most trusted gaming platform in East Africa! 🇰🇪</p>

        <h3>1. ACCEPTANCE OF TERMS</h3>
        <p>By accessing Harambee Cash, you enter into a legally binding agreement. We've helped over 50,000 Kenyans play safely and responsibly.</p>

        <h3>2. ELIGIBILITY</h3>
        <ul>
            <li><strong>✓ Age Requirement:</strong> 18+ years (ID verification available)</li>
            <li><strong>✓ Location:</strong> Open to all Kenyan residents with M-Pesa access</li>
            <li><strong>✓ One Account:</strong> Limited to one account per person</li>
        </ul>

        <h3>3. GAME RULES & FAIR PLAY</h3>
        <ul>
            <li><strong>Minimum Play:</strong> Ksh 1.00</li>
            <li><strong>Game Frequency:</strong> Every 30 seconds, 24/7</li>
            <li><strong>Platform Fee:</strong> 10% (Lowest in Kenya!)</li>
            <li><strong>Winnings:</strong> Paid instantly to your wallet</li>
        </ul>

        <h3>4. FINANCIAL TERMS</h3>
        <ul>
            <li><strong>Deposits:</strong> Via M-Pesa PayBill (Coming Soon)</li>
            <li><strong>Withdrawals:</strong> To M-Pesa within 5 minutes</li>
            <li><strong>Minimum Withdrawal:</strong> Ksh 50</li>
            <li><strong>No Hidden Fees:</strong> What you see is what you get</li>
        </ul>

        <h3>5. ACCOUNT SECURITY</h3>
        <ul>
            <li><strong>2FA Available:</strong> Extra layer of security</li>
            <li><strong>PIN Protection:</strong> Transaction PIN required</li>
            <li><strong>24/7 Monitoring:</strong> Fraud detection active</li>
        </ul>

        <div class="legal-footer">
            <p style="font-size: 20px; font-weight: bold;">📞 Need Help? Contact Our Legal Team</p>
            <p>legal@harambee-cash.co.ke | 24/7 Support Line: +254 700 000000</p>
            <p style="margin-top: 15px;">© 2025 Pigasimu. All Rights Reserved. Licensed in Kenya.</p>
        </div>
